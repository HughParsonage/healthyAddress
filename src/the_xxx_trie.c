...
int THE_xxx3(TrieNode *root, WordData wd, uint16_t p_postcode /* problem postcode */) {
  const char *x = wd.x;
  int n_words = wd.n_words;
...
SEXP C_do_the_xxx(SEXP x, SEXP Postcode) {
  ...
  insert_all(root);
  uint16_t * problem_postcodes = calloc(SUP_POSTCODES, sizeof(uint16_t));
  for (int p = 0; p < N_XXXPOSTCODE; ++p) {
    problem_postcodes[THE_XXX_COMPLX[p].postcode] = p + 1;
  }
  ...
  for (R_xlen_t i = 0; i < N; ++i) {
    ...
    int postcode = postcode_was_null ? 1 : postcodep[i];
